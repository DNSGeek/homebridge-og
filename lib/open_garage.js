function OpenGarageModule(
  ...
  class OpenGarage {
    ...
    isClosed() {
      ...
      else throw new Error("Last poll failed - " + this.currentState.error);
    }
    ...
    targetDoorState() {
      if (
        ...
        // past time expected to open
        if (this.isClosed()) return Characteristic.TargetDoorState.CLOSED;
        else return Characteristic.TargetDoorState.OPEN;
      } else {
    ...
    currentDoorState() {
      ...
      else return Characteristic.CurrentDoorState.OPEN;
    }
    ...
    currentVehicleState() {
      ...
      else return Characteristic.OccupancyDetected.OCCUPANCY_NOT_DETECTED;
    }
    ...
    changeState(state, callback) {
      ...
      openGarageApi
        .setTargetState(targetStateClosed)
        .then(
          (_) => {
