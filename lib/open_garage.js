  ...
  class OpenGarage {
    ...
    updateReportedDoorState() {
      ...
      const userInitiated =
        ...
        Date.now() - this.lastTarget.ts < openCloseDurationMs * 4;
      if (userInitiated || this.pendingClosed === newClosed) {
        this.reportedClosed = newClosed;
        this.pendingClosed = null;
      } else {
    ...
    currentDoorState() {
      if (this.isReportedClosed()) return Characteristic.CurrentDoorState.CLOSED;
